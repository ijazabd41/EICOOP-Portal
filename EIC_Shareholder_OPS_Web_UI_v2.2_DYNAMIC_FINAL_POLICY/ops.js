const BASE_URL = window.EIC_OPS_BASE_URL || '';
...
function shell(content){
  ...
  root.innerHTML = `<div class="shell"><header class="topbar"><div><h1>EIC Shareholder Operations</h1><div class="role">${role}</div></div><div>${esc(session?.user?.name||'')}</div></header><main class="content">${content}</main></div>`;
}
...
async function openTransfer(id,backKey='transfers'){
  try{
    ...
    if(r.can_operations_review){a.innerHTML+=`<button data-act="ops-approve">Approve & Send to Receiver</button><button class="danger" data-act="ops-reject">Operations Reject</button>`;}
    if(r.can_chairman_approve){a.innerHTML+=`<button class="gold" data-act="chair-approve">Chairman Approve</button><button class="danger" data-act="chair-reject">Chairman Reject</button>`;}
    if(r.can_complete){a.innerHTML+=`<button data-act="complete">Complete Transfer</button>`;}
...
async function openApplication(id,backKey='applications'){
  ...
  document.getElementById('back').onclick=()=>loadSection(backKey);
  const a=document.getElementById('actions');
  if(session.permissions.membership_review){
    if(r.state==='submitted') a.innerHTML+='<button data-act="document_review">Documents Reviewed</button>';
    if(r.state==='document_review') a.innerHTML+='<button data-act="finance">Send to Finance</button>';
    if(['submitted','document_review','finance'].includes(r.state)) a.innerHTML+='<button data-act="approve">Approve Membership</button><button class="danger" data-act="reject">Reject</button>';
  }
