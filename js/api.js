...
const API = ((_DB='production', SK='cd_session', NOTIFY='eicoopit@gmail.com') => {
  ...
  // ── CDN CONFIGURATION ──────────────────────────────────────────
  // Set to 'https://cdn.coop-discounts.com' to serve images via CDN
  const CDN_URL = ''; 
  const IMG_BASE = CDN_URL || PX;
  const DB = _DB;
  ...
  const partnerImg = pid=> `${IMG_BASE}/web/image/res.partner/${pid}/image_1920`;
  const bannerImg  = id => `${IMG_BASE}/web/image/deal.day.slider/${id}/banner_image`;
  ...
  function mkUrl(path, p={}) {
    ...
    if (typeof path === 'string' && !path.startsWith('/api/shareholder/')) {
  ...
  async function extractError(r, path, method = 'GET', ms = 0) {
    ...
    // Auto-clear session if switching environments causes a DB miss
    if (/database.*does not exist/i.test(rawBody) || /FATAL: database/i.test(rawBody)) {
      try {
        localStorage.removeItem(SK);
        localStorage.removeItem('cd_session_id');
        sessionStorage.removeItem(SK);
      } catch(e) {}
      msg = 'Environment changed (database not found). Your old session was cleared. Please refresh the page.';
    }
    ...
    const requestPromise = (async () => {
      ...
      if (!d || d.success === 0 || d.success === false || d.success === "" || d.error) {
        ...
        if (d && d.error && d.error.data && d.error.data.message) {
          msg = d.error.data.message;
        } else if (d && (d.message || d.error)) {
          msg = d.message || d.error;
        } else if (d && d.msg) {
          msg = d.msg;
        }
        
        // Permissive check for Odoo's weird OTP success responses
        if (/otp.*(sent|generated|created|attempt)/i.test(msg) || /sent.*otp/i.test(msg)) {
          return d;
        }
        
        msg = normalizeApiErrorMessage(typeof msg === 'string' ? msg : JSON.stringify(msg));
        Log.apiFail(method, path, r.status, ms, new Error(msg));
        const e = new Error(msg);
        e.isAccessError = /not allowed to access/i.test(msg);
        e.rawError = d ? d.error : null;
        notifyError(path, e);
        throw e;
      }
  ...
  async function loginWithSSO(provider, token) {
    clearCache();
    const url = `https://coop-discounts.com/api/auth/${provider}`;
    try {
      const resp = await fetch(url, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        // Sending token in multiple fields to handle different backend expectations until confirmed
        body: JSON.stringify({ token: token, access_token: token, credential: token })
      });
      if (!resp.ok) {
        const text = await resp.text();
        return { ok: false, err: `Authentication failed: ${resp.status} ${text.substring(0, 50)}` };
      }
  ...
  async function login(emailOrPhone, password) {
    ...
    let dbName = DB;
    try {
      const confRes = await fetch(API.PX + '/api/config');
      if (confRes.ok) {
         const conf = await confRes.json();
         if (conf.db) dbName = conf.db;
      }
    } catch (e) { /* ignore and use default */ }

    const d = await POST('/web/session/authenticate', {
      jsonrpc:'2.0', method:'call', id:1,
      params: { db:dbName, login:emailOrPhone, password }
    });
  ...
  async function createOrder() {
    ...
    // Explicitly hit the GET order API immediately so we fetch global minimum_order_amount
    setTimeout(() => { getOrder(rec.id).catch(()=>{}); }, 200);
  ...
  async function getOrder(id) {
    try {
      const res = await GET(`/api/order/${id}?by_AJR=1`);
      
      // Globally update minimum order amount if provided in the response
      if (res && res.data) {
        const oData = Array.isArray(res.data) ? res.data[0] : res.data;
        let newMin = window._cd_min_order_amount;
        if (oData && typeof oData.min_order_amount !== 'undefined') {
          newMin = parseFloat(oData.min_order_amount);
        } else if (oData && typeof oData.minimum_order_amount !== 'undefined') {
          newMin = parseFloat(oData.minimum_order_amount);
        }
        
        if (newMin !== undefined && newMin !== window._cd_min_order_amount) {
          window._cd_min_order_amount = newMin;
      ...
      if (isOdooAccessError(e) || e.isAccessError || /500|Server error/i.test(e.message)) {
        try {
          const fallback = await GET('/api/order', { domain: `[('id','=',${id})]`, by_AJR: 1 });
          if (fallback && fallback.data && fallback.data.length > 0) {
            const oData = fallback.data[0];
            let newMin = window._cd_min_order_amount;
            if (oData && typeof oData.min_order_amount !== 'undefined') {
              newMin = parseFloat(oData.min_order_amount);
            }
            if (newMin !== undefined && newMin !== window._cd_min_order_amount) {
              ...
              else if (typeof renderDrawer === 'function') renderDrawer();
            }
  ...
  async function createTx(oid, provId) {
    ...
    try {
      ...
      const txIds = orderTransactions(oRec);
      if (txIds && txIds.length > 0) {
        const txId = txIds[txIds.length - 1];
        Log.info('Payment', 'createTx ✓ (reusing existing)', { orderId: oid, providerId: provId, txId });
      ...
      // Hit the native Odoo checkout controller
      await POST('/payment/transaction' + qs, {
        jsonrpc: '2.0',
        method: 'call',
        params: {
          provider_id: parseInt(provId, 10),
          payment_option_id: parseInt(provId, 10),
          amount: amount,
      ...
      const txIds2 = orderTransactions(oRec2);
      if (!txIds2 || txIds2.length === 0) throw new Error("Transaction was not generated by Odoo.");
      
      const txId = txIds2[txIds2.length - 1];
  ...
  // ── TELR PAYMENT GATEWAY (Secure Backend Flow) ──────────────────
  const GATEWAY_URL = '/gateway_api';
  ...
  async function createTelrSession(orderId, amount, currency, description, customerInfo) {
    ...
    const resp = await fetch(GATEWAY_URL + '/telr_create.php', {
      method: 'POST',
      headers: headers,
      body: JSON.stringify(body)
    });
    
    if (!resp.ok) throw new Error('Telr session creation failed on server (HTTP ' + resp.status + ')');
    const data = await resp.json();
    if (!data.success || !data.payment_url) {
      Log.error('Telr', 'createSession — invalid response', data);
      throw new Error(data.message || 'Telr did not return a payment session.');
    }
    Log.info('Telr', 'createSession ✓', { ref: data.ref, url: data.payment_url });
    return { ref: data.ref, url: data.payment_url };
  }

  /**
   * Create a Tamara hosted payment session.
   * @param {number} orderId – Odoo order ID
   * @param {number|string} amount – Total amount
   * @param {string} currency – e.g. 'AED'
   * @param {string} description – e.g. 'Order S00308'
   * @returns {{ checkout_url: string, tamara_order_id: string }}
   */
  async function createTamaraSession(orderId, amount, currency, description) {
    Log.info('Tamara', 'createSession → start', { orderId, amount, currency });
    const body = {
      order_id: String(orderId),
      amount: String(parseFloat(amount).toFixed(2)),
      currency: currency || 'AED',
    ...
    const resp = await fetch(GATEWAY_URL + '/tamara_create.php', {
      method: 'POST',
      headers: headers,
      body: JSON.stringify(body)
    });
    
    if (!resp.ok) throw new Error('Tamara session creation failed on server (HTTP ' + resp.status + ')');
    const data = await resp.json();
    if (!data.success || !data.checkout_url) {
      Log.error('Tamara', 'createSession — invalid response', data);
      throw new Error(data.message || 'Tamara did not return a checkout session.');
    }
    Log.info('Tamara', 'createSession ✓', { url: data.checkout_url });
    return { checkout_url: data.checkout_url, tamara_order_id: data.tamara_order_id };
  }
  ...
  async function verifyTelrPayment(telrRef) {
    ...
    const resp = await fetch(GATEWAY_URL + '/telr_verify.php', {
      method: 'POST',
      headers: headers,
      body: JSON.stringify({ ref: telrRef })
    });
    
    if (!resp.ok) throw new Error('Failed to verify payment with server.');
    const data = await resp.json();
    ...
    // Convert to legacy structure format expected by caller
    return { 
      code: data.status === 'Paid' ? 3 : 1, 
      text: data.status 
    };
  }
  ...
  async function confirmOrderPayment(orderId, preferredProviderId) {
    ...
    if (isCod) {
      Log.info('Payment', 'Confirming order for Cash on Delivery', { orderId: oid });
      try {
        await GET(`/api/order/${oid}/order_confirm?by_AJR=1`);
      } catch (e) {
        Log.warn('Payment', 'order_confirm failed for COD', { orderId: oid, message: e.message });
      }
  ...
  async function getCsrfToken() {
    try {
      const res = await fetch(mkUrl('/web/login'));
      const text = await res.text();
      const match = text.match(/csrf_token:\s*"([^"]+)"/);
  ...
  const updContact = async (pid, p) => {
    if (p.image_1920) {
      const csrf = await getCsrfToken();
      if (csrf) p.csrf_token = csrf;
        ...
        if (v !== undefined && v !== null && v !== '') {
          if (k === 'image_1920') {
             try {
               ...
               formData.append(k, blob, 'profile.jpg');
             } catch(e) {
               formData.append(k, v);
             }
          } else {
      ...
      const url = mkUrl(`/api/contacts/${pid}/update`, { by_AJR: 1 });
      const h = hdrs();
      delete h['Content-Type']; // Let browser set Content-Type for multipart/form-data
      
      Log.apiStart('POST', `/api/contacts/${pid}/update`, Object.keys(p));
      const t0 = performance.now();
      ...
      const ms = Math.round(performance.now() - t0);
      if (!res.ok) return extractError(res, `/api/contacts/${pid}/update`, 'POST', ms);
      const d = await res.json();
      if (d.success === 0 || d.error) {
         let msg = d.error?.data?.message || d.message || d.error || 'API error';
         throw new Error(normalizeApiErrorMessage(msg));
      }
      Log.apiDone('POST', `/api/contacts/${pid}/update`, res.status, ms, d);
      return d;
    }
    return GET(`/api/contacts/${pid}/update`, p);
  };
  ...
  // ── COUNTRIES & STATES ────────────────────────────────────────
  const getCountries = ()  => GET('/api/country', { limit: 300 }).then(r => {
  ...
  async function confirmGuestOrder(providerId) {
    ...
    if (isAbsoluteUrl(redirectUrl) || redirectUrl.includes('telr.com')) {
        ...
        return { success: true, orderName: 'S' + Math.floor(Math.random() * 90000 + 10000), redirectUrl: redirectUrl, amount: amount };
    }
    ...
    return { success: true, orderName, amount: amount };
  }
  ...
  return {
    ...
    // Auth
    login, loginWithSSO, logout, register, updatePassword, forgotPassword,
    ...
    isTelrProvider,
    createTelrSession, verifyTelrPayment,
    createTamaraSession,
    // Invoices
...
// Fetch global website configuration on load
if (typeof window !== 'undefined') {
  fetch(API.PX + '/api/config')
    .then(r => r.json())
    .then(conf => {
      if (conf.min_order_amount && typeof window._cd_min_order_amount === 'undefined') window._cd_min_order_amount = conf.min_order_amount;
      if (conf.free_delivery_amount && typeof window._cd_free_delivery_amount === 'undefined') window._cd_free_delivery_amount = conf.free_delivery_amount;
    })
    .catch(err => API.log.error('Config', 'Failed to fetch global configuration', err));
}
